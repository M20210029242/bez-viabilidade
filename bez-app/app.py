...
Calcula indicadores de viabilidade (VPL, Índice de Lucratividade)
...
def gerar_recomendacao(resultado):
    ...
    if pontos_positivos == 2:
        return "🟢", "Projeto recomendado", COR_POSITIVO, \
            "O projeto apresenta VPL positivo e índice de lucratividade favorável."
...
emoji, titulo_rec, cor_rec, texto_rec = gerar_recomendacao(resultado)
...
col5.metric("VPL", f"R$ {resultado['vpl']:,.2f}")
il_txt = f"{resultado['indice_lucratividade']:.2f}" if resultado["indice_lucratividade"] is not None else "N/A"
col6.metric("Índice de Lucratividade", il_txt)
...
with aba_cenarios:
    ...
    for col, (nome_cenario, percentual) in zip(cols_cenarios, cenarios.items()):
        ...
        emoji_c, titulo_c, cor_c, _ = gerar_recomendacao(res_cenario)
        il_c = f"{res_cenario['indice_lucratividade']:.2f}" if res_cenario["indice_lucratividade"] is not None else "N/A"
        ...
        with col:
            st.markdown(
                ...
                <div style="border: 1px solid #E0E0E0; border-radius: 10px; padding: 16px;">
                    ...
                    <p>Índice de Lucratividade: <b>{il_c}</b></p>
